// fb6hil.c
// Drawings with frame buffer, Hilbert curves
//
// 2016-06-24	PV	First version
...
void dr(char *s, int color) {
	for (char c ; (c = *s++) ; ) {
		if (c=='-')
			...
			draw_line_bresenham(cx, cy, nx, ny, color);
			cx = nx;
			cy = ny;
		}
	}
...
void ls2(int level, char *rule, int color) {
	if (level==0)
		dr(rule, color);
	else
		for (char c ; (c = *rule++) ; ) {
			if (c=='X')
				ls2(level-1, "-YF+XFX+FY-", color);
			else if (c=='Y')
				ls2(level-1, "+XF-YFY-FX+", color);
			else {
				char sb[2];
				sb[0] = c;
				sb[1] = 0;
...
void draw(int level, int color) {
	int side = (int)(pow(2.0, (double)level)-1);	// Total number of elemental elements on the side
	mod = (int)((vinfo.yres-20)/(side+1));			// Size of one side

	// Initial coordinates
	cx = (vinfo.xres-mod*side)/2;
	cy = vinfo.yres - (vinfo.yres-mod*(side))/2;
	a = 0;											// Initial angle
