// fb5moi.c
// Drawings with frame buffer, Moires patterns
...
#define min(a,b) ((a<b)?(a):(b))
	int r = 0.95*min(vinfo.yres, vinfo.xres/2);
	//r &= ~1;
	r |= 1;
	int x1 = ((vinfo.xres/2)-r)/2;
	int y = (vinfo.yres-r)/2;
	int x2 = x1+vinfo.xres/2;

	clear_screen(0);

	// Moires
	for (int i=0 ; i<r ; i+=2) {
		draw_line_bresenham(x1+i, y, x1+r-i, y+r, GREEN);
		draw_line_wu(x2+i, y, x2+r-i, y+r, GREEN);
		draw_line_bresenham(x1+i+1, y, x1+r-i-1, y+r, BLACK);
		draw_line_wu(x2+i+1, y, x2+r-i-1, y+r, BLACK);

		draw_line_bresenham(x1, y+i, x1+r, y+r-i, GREEN);
		draw_line_wu(x2, y+i, x2+r, y+r-i, GREEN);
		draw_line_bresenham(x1, y+i+1, x1+r, y+r-i-1, BLACK);
		draw_line_wu(x2, y+i+1, x2+r, y+r-i-1, BLACK);
	}
}
